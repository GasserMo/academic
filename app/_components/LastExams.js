...
function LastExams({ lastExams }) {
    ...
    return (
        <div>
            {lastExams.map((exam) => (
                <Link key={exam._id} href={role === 'teacher' ? `/Teacher/Exams/details/${exam._id}` : `/Student/Exams/submission/${exam._id}`}>
                    <div
                        ...
                        <p className="text-black font-poppins over">{exam.questions?.length} Questions</p>
                    </div>
                </Link>
            ))}
        </div>
