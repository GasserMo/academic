...
function ParentReportDetails({ selectedReportId }) {
    ...
    const handleReplySubmit = async (e) => {
        ...
        try {
            ...
            setReportDetails((prev) => ({ ...prev, reply: message }));
            setReplyMessage('');
        } catch (err) {
        ...
        mt-3 mx-5 min-h-[20vh] max-h-[100vh] md:min-h-[80vh] bg-white rounded-2xl">
            <div className="flex m-3 items-start flex-col">
                <div className="flex flex-col ml-3">
                    ...
                    <p className="font-poppins text-[10px] text-gray-500">{`From: ${from.name.first} ${from.name.last}`}</p>
                </div>
            ...
            <div>
                <div className="flex flex-col justify-between px-4 space-y-1">
                    <div className="flex flex-col bg-blueHover p-3 rounded-2xl">
                        <p className="font-poppins text-[15px] font-semibold">Message:</p>
...
export default ParentReportDetails
