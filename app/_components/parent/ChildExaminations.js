...
const ChildExaminations = ({ data }) => {
    ...
    return (
        <div className="">
            {data?.length > 0 ? (
                data.map((exam, index) => (
                    <ExamDetails key={index} title={exam.title} startDate={exam.startDate}
                        endDate={exam.endDate} duration={exam.duration} score={exam.score}
                        status={exam.status}
                    />
                ))
            ) : (
                <p className="flex items-center justify-center font-poppins">No examinations available.</p>
            )}
        </div>
    );
};
...
function ExamDetails({ title, status, score, duration, endDate, startDate }) {
        ...
        <div className="flex    flex-col my-4 bg-[#EBF4F7] shadow-lg shadow-gray-300 p-2 space-y-2 rounded-xl">
            ...
            <div className="flex-col">
                <div className="flex justify-between">
                    <p className="font-poppins text-[18px] font-bold ">{title}</p>
